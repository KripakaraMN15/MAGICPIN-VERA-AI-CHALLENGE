...
LLM_API_KEY = os.environ.get("OPENAI_API_KEY", "")  # set env var; don't paste secrets here

# Model to use (leave empty for default, or specify like "gpt-4o", "claude-3-5-sonnet-20241022", etc.)
LLM_MODEL = os.environ.get("OPENAI_MODEL", "")  # optional
...
def print_score_bar(dimension: str, score: int, max_score: int = 10):
    ...
    print(f"  {dimension:22} [{color}{bar}{Colors.RESET}] {color}{score:2}/{max_score}{Colors.RESET}")

def print_reason(text: str):
    safe = text.encode("ascii", "replace").decode("ascii")
    wrapped = safe[:200] + "..." if len(safe) > 200 else safe
...
class OpenAIProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        if self.model.startswith(("gpt-5", "o1", "o3", "o4")):
            body_dict["max_completion_tokens"] = 3000
        else:
            body_dict["temperature"] = 0.2
            body_dict["max_tokens"] = 1500
        if "json" in (prompt + " " + str(system or "")).lower():
